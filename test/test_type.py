...
class TestType:
    ...
    def test_allow_nan_in_structures(self):
        ...
        assert (
            orjson.dumps({"x": float("Infinity")}, allow_nan=True) == b'{"x":Infinity}'
        )
    ...
    def test_allow_nan_numpy(self):
        ...
        assert (
            orjson.dumps(
                arr_with_nan, option=orjson.OPT_SERIALIZE_NUMPY, allow_nan=True
            )
            == b"[1.0,NaN,3.0]"
        )
        assert (
            orjson.dumps(
                arr_with_inf, option=orjson.OPT_SERIALIZE_NUMPY, allow_nan=True
            )
            == b"[1.0,Infinity,-Infinity]"
        )
