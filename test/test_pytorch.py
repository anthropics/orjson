...
class PyTorchTests(unittest.TestCase):
    ...
    def test_tensor_zero_dim(self):
        ...
        self.assertEqual(
            orjson.dumps(tensor_float, option=orjson.OPT_SERIALIZE_NUMPY),
            b"0.03",
        )

        tensor_int = torch.tensor(42)
        self.assertEqual(
            orjson.dumps(tensor_int, option=orjson.OPT_SERIALIZE_NUMPY),
            b"42",
        )
    ...
    def test_tensor_special_values(self):
        ...
        self.assertEqual(
            orjson.dumps(tensor_nan, option=orjson.OPT_SERIALIZE_NUMPY),
            b"NaN",
        )

        tensor_inf = torch.tensor(float("inf"))
        self.assertEqual(
            orjson.dumps(tensor_inf, option=orjson.OPT_SERIALIZE_NUMPY),
            b"Infinity",
        )
    ...
    def test_tensor_in_list(self):
        ...
        assert (
            orjson.dumps([torch.tensor([1, 2])], option=orjson.OPT_SERIALIZE_NUMPY)
            == b"[[1,2]]"
        )
