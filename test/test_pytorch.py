...
class PyTorchTests(unittest.TestCase):
    def test_tensor_1d(self):
        ...
        self.assertEqual(
            orjson.dumps(tensor, option=orjson.OPT_SERIALIZE_NUMPY), b"[1,2,3]"
        )
    ...
    def test_tensor_2d(self):
        ...
        self.assertEqual(
            orjson.dumps(tensor, option=orjson.OPT_SERIALIZE_NUMPY), b"[[1,2],[3,4]]"
        )
    ...
    def test_tensor_float(self):
        ...
        self.assertEqual(
            orjson.dumps(tensor, option=orjson.OPT_SERIALIZE_NUMPY), b"[1.1,2.2,3.3]"
        )
    ...
    def test_tensor_requires_grad(self):
        ...
        self.assertEqual(
            orjson.dumps(tensor, option=orjson.OPT_SERIALIZE_NUMPY), b"[1.0,2.0,3.0]"
        )
    ...
    def test_tensor_on_gpu_and_requires_grad(self):
        ...
        if not torch.cuda.is_available():
            self.skipTest("CUDA not available")
        tensor = torch.tensor([1.0, 2.0, 3.0], requires_grad=True).cuda()
        self.assertEqual(
            orjson.dumps(tensor, option=orjson.OPT_SERIALIZE_NUMPY), b"[1.0,2.0,3.0]"
        )
    ...
    def test_tensor_zero_dim(self):
        ...
        self.assertEqual(
            orjson.dumps(tensor_float, option=orjson.OPT_SERIALIZE_NUMPY), b"0.03"
        )

        # Test int scalar tensor
        tensor_int = torch.tensor(42)
        self.assertEqual(
            orjson.dumps(tensor_int, option=orjson.OPT_SERIALIZE_NUMPY), b"42"
        )
    ...
    def test_tensor_special_values(self):
        ...
        self.assertEqual(
            orjson.dumps(tensor_nan, option=orjson.OPT_SERIALIZE_NUMPY), b"NaN"
        )

        # Test inf
        tensor_inf = torch.tensor(float("inf"))
        self.assertEqual(
            orjson.dumps(tensor_inf, option=orjson.OPT_SERIALIZE_NUMPY), b"Infinity"
        )
        tensor_neg_inf = torch.tensor(float("-inf"))
        self.assertEqual(
            orjson.dumps(tensor_neg_inf, option=orjson.OPT_SERIALIZE_NUMPY),
            b"-Infinity",
        )
